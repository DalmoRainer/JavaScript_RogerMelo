...
const getOddNumber = numero => numero % 2 != 0;

const numerosImpar = randomNumbers.filter(getOddNumber);
...
const getAbaixoDe501 = (accumulator, crazyNumber) => crazyNumber < 501 ? ++accumulator : accumulator;

const abaixoDe501 = crazyNumbers.reduce(getAbaixoDe501, 0);
...
const numeroAoQuadrado = numbers.map(quadrado => quadrado ** 2);
...
const antesDe2000 = tarantinoMovies.filter(tarantinoMovie => tarantinoMovie.release < 2000);

console.log(antesDe2000);
...
const nomesSeries = tvShows.map(tvShow => tvShow.name);
...
// O RETURN JÁ ESTÁ IMPLÍCITO E POR ISSO NÃO É NECESSÁRIO DECLARÁ-LO
// NÃO É NECESSÁRIO COLOCAR O JOGO NEM O JOGO.NAME
